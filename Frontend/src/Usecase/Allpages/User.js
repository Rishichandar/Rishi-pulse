...
function User(){
  ...
  const searchId = useCallback(async () => {
    try {
      const response = await fetch(`http://localhost:4023/project_info/${userId}`);//http://localhost:3000/projectdetails/:ID
      if (!response.ok) {
        throw new Error('Failed to fetch user data');
      }
      const data = await response.json();
...
//UPADTE INPUT value using usestate
  ...
  //for project-title to get user data  
    ...
    const searchProject = useCallback(async () => {
      try {
        const response = await fetch(`http://localhost:4023/project_info1/${projectTitle}`);//http://localhost:3000/projectdetails/:ID
        if (!response.ok) {
          throw new Error("failed  to fetch");
